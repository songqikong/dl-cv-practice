...
def calibrate_laser_plane(image_path, chessboard_size, square_size):
    # 读取图片
    ...
    if ret:
        ...
        # 使用solvePnP函数来计算外参矩阵
        _, rvec, tvec = cv2.solvePnP(world_points, image_points, camera_matrix,
                                     None)
...
if __name__ == "__main__":
    ...
    # 棋盘格大小
    chessboard_size = (6, 8)  # 根据实际情况设置
    square_size = 0.02  # 根据实际情况设置，单位为米

    image_path = "./16.bmp"  # 根据实际情况设置图片路径
    laser_plane_params = calibrate_laser_plane(image_path, chessboard_size,
                                               square_size)

    print("激光平面参数:", laser_plane_params)